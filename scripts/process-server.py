...
def process_IPC1_trace(trace_file):
    ...
    command = f"./scripts/run_champsim.sh ./IPC1/All_imps/{trace_file} > {output_file}"
    ...
    if result.stdout == "OK\n":
        ...
        with open('trace-names/server_memory_intensive_traces.txt', 'a') as file:
...
def process_CVP1_trace(trace_file):
    ...
    command = f"./scripts/run_champsim.sh ./CVP1/All_imps/{trace_file} > {output_file}"
    subprocess.run(command, shell=True)
...
if __name__ == "__main__":
    ...
    trace_filenames = os.listdir("IPC1/All_imps")
    ...
    with Pool(processes=num_processes) as pool:
        with tqdm(total=len(trace_filenames), desc="Processing IPC1 trace files", unit="trace", bar_format=custom_format) as pbar:
            for _ in pool.imap_unordered(process_IPC1_trace, trace_filenames):
                pbar.update()
    
    trace_filenames = os.listdir("CVP1/All_imps")

    # Use tqdm to display a progress bar
    with Pool(processes=num_processes) as pool:
        with tqdm(total=len(trace_filenames), desc="Processing CVP1 trace files", unit="trace", bar_format=custom_format) as pbar:
            for _ in pool.imap_unordered(process_CVP1_trace, trace_filenames):
                pbar.update()
